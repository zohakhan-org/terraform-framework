...
AWS_REGION=$(yq e '.aws_region' "$CONFIG_FILE")
...
yq e  '.services | keys[]' "$CONFIG_FILE" | while read -r SERVICE; do
  case $SERVICE in
    ecs)
      ...
      ECS_CLUSTER_NAME=$(yq e '.services.ECS.cluster_name' "$CONFIG_FILE")
      ECS_INSTANCE_TYPE=$(yq e '.services.ECS.instance_type' "$CONFIG_FILE")
    ...
    s3)
      ...
      S3_BUCKET_NAME=$(yq e '.services.S3.bucket_name' "$CONFIG_FILE")
      echo "s3_bucket_name = \"$S3_BUCKET_NAME\"" >> "$TFVARS_FILE"
      ;;
    iam)
      echo "Adding IAM configuration..."
      IAM_ROLES=$(yq e '.services.iam.roles' "$CONFIG_FILE")
      IAM_GROUPS=$(yq e '.services.iam.groups' "$CONFIG_FILE")
