...
yq   '.services | keys[]' "$CONFIG_FILE" | while read -r SERVICE; do
  case $SERVICE in
    ecs)
      if yq '.services.ecs' "$CONFIG_FILE" > /dev/null 2>&1; then
        echo "Adding ECS configuration..."
        ECS_CLUSTER_NAME=$(yq  '.services.ECS.cluster_name' "$CONFIG_FILE")
        ECS_INSTANCE_TYPE=$(yq  '.services.ECS.instance_type' "$CONFIG_FILE")
        echo "ecs_cluster_name = \"$ECS_CLUSTER_NAME\"" >> "$TFVARS_FILE"
        echo "ecs_instance_type = \"$ECS_INSTANCE_TYPE\"" >> "$TFVARS_FILE"
      fi
      ;;
    s3)
      if yq  '.services.s3' "$CONFIG_FILE" > /dev/null 2>&1; then
        echo "Adding S3 configuration..."
        S3_BUCKET_NAME=$(yq  '.services.S3.bucket_name' "$CONFIG_FILE")
